...
/**
 * JAMB ePIN adapter. Plug real vendor endpoint/params once credentials are issued.
 */
class JambProvider extends BaseServiceProvider {
  constructor() {
    super();
    this.baseUrl = env.services.jamb.baseUrl;
    this.apiKey = env.services.jamb.apiKey;
  }
  ...
  async purchase(service, inputPayload, order) {
    try {
      ...
      logger.error(`JAMB ePIN provider purchase failed: ${err.message}`);
      return { success: false, providerRef: null, token: null, raw: { error: err.message } };
    }
  }
}

module.exports = JambProvider;
