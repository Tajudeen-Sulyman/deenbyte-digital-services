...
/**
 * BVN Verification adapter. Plug real vendor endpoint/params once credentials are issued.
 */
class BvnProvider extends BaseServiceProvider {
  constructor() {
    super();
    this.baseUrl = env.services.bvn.baseUrl;
    this.apiKey = env.services.bvn.apiKey;
  }
  ...
  async purchase(service, inputPayload, order) {
    try {
      ...
      logger.error(`BVN Verification provider purchase failed: ${err.message}`);
      return { success: false, providerRef: null, token: null, raw: { error: err.message } };
    }
  }
}

module.exports = BvnProvider;
